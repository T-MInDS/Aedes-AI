...
if __name__ == '__main__':
    ...
    base=np.array(["LSTM"])
    hi=np.array(["LSTM'"])
    lo=np.array(["LSTM_2000"])
    both=np.array(["LSTM_120"])
    ...
    for i in range(0,2):
        for j in range(0,2):
            ax[i,j].bar(x,base_mean[orders[count],:],yerr=base_stds[orders[count],:],width=0.1,color=c1,label='LSTM')
            ax[i,j].bar(x+0.1,hi_mean[orders[count],:],yerr=hi_stds[orders[count],:],width=0.1,color=c2,label='LSTM\'')
            ax[i,j].bar(x+0.2,lo_mean[orders[count],:],yerr=lo_stds[orders[count],:],width=0.1,color=c3,label='LSTM$_{2000}$')
            ax[i,j].bar(x+0.3,both_mean[orders[count],:],yerr=both_stds[orders[count],:],width=0.1,color=c4,label='LSTM$_{120}$')
            ax[i,j].set_xticks([])
            count+=1
